...
> Runbook for the Rev 2 three-interface burn-in. Prepared against `c78dd7ed4`.
...
- [x] UI 3 Playwright smoke: 4/4 green (`c78dd7ed4`).
- [x] D8.5 remote-bridge pen-test harness: static origin-policy and source/capability gates signed off.
- [x] UI 3 12-locale parity and Anemos theme gate: green.
- [x] Combined iOS and Android shell builds: green; selector, Classic, Chamber Full route, and Anemos Chamber assets present.
...
Observed current-tree asset sizes:

| Artifact | `du -sh` |
|---|---:|
| `packages/ios/WebAssets` | 71M |
| `packages/android/dist` | 71M |
...
| Selector | [ ] | [ ] | Deep links route to remembered UI 2/3, or UI 3 when UI 1 is remembered; UI 1 is never used for an `opencode://` link. | |
...
| UI 3 — Anemos Chamber | [ ] | [ ] | Boot, Basic auth, and live SSE updates work against the opencode server. | |
| UI 3 — Anemos Chamber | [ ] | [ ] | Markdown worker loads from the `tauri://localhost` custom scheme. | |
| UI 3 — Anemos Chamber | [ ] | [ ] | Cold-start `opencode://` deep link opens the UI 3 target and the intended session/location. | |
| UI 3 — Anemos Chamber | [ ] | [ ] | Haptics and share actions work from the native shell. | |
...
| UI 2 — Classic | [ ] | [ ] | Spot-check launch, server connection, session list, prompt, streaming, and resume behavior against today’s app. | |
| UI 3 — Anemos Chamber | [ ] | [ ] | Boot, Basic auth, and live SSE updates work against the opencode server. | |
| UI 3 — Anemos Chamber | [ ] | [ ] | Markdown worker loads under Android WebView CSP and the `http://tauri.localhost` origin. | |
| UI 3 — Anemos Chamber | [ ] | [ ] | Cold-start `opencode://` deep link opens the UI 3 target and the intended session/location. | |
| UI 3 — Anemos Chamber | [ ] | [ ] | Haptics and share actions work from the native shell. | |
| Cross-cutting | [ ] | [ ] | Repeatedly switch selector → UI 1 → UI 2 → UI 3; memory returns to baseline with one WebView runtime at a time. | |
